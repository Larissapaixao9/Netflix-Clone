...
## Constantes padrao
    ...
    if H < 0:
        print('Cuidado, altitude negativa. O modelo ISA é inválido. Resultados para H=0.')
        T = TB[0]
        p = PB[0]
        rho = p / (R * T)
        a = np.sqrt(gama * R * T)
        ...
        if H > 50000:
            print('Cuidado, H>50.000 m. O modelo ISA é inválido. Resultados para H=50.000 m.')
            T = TB[4]
